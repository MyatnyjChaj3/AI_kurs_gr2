...
### 7. `POST /head` – предварительный просмотр данных

Возвращает первые несколько строк загруженного CSV-файла в виде списка JSON-объектов.

**Параметры:**  
    n (query-параметр) — количество строк для вывода (по умолчанию 5).
...
```bash
curl -X POST "http://127.0.0.1:8000/head?n=10" \
  -F "file=@data/example.csv"
```

##Логирование

В сервисе реализовано логирование в формате JSON. Все запросы к API фиксируются в локальном файле для последующего анализа.

- Путь к логам: logs/api.log

- Формат записи: каждая строка является валидным JSON-объектом.

**Состав лога:**

- timestamp: время обработки запроса (UTC);

- request_id: уникальный идентификатор запроса (UUID);

- endpoint: путь вызванного метода;

- status: HTTP-статус ответа;

- latency_ms: время выполнения запроса на сервере;

- extra: дополнительные метаданные (например, количество строк или параметры запроса).

**Пример лога:**

```json
{"timestamp": "2025-12-23T17:36:58.615981", "request_id": "e2651e97-608b-476d-ad07-426e880df66d", "endpoint": "/head", "status": 200, "latency_ms": 22.0, "n": 5}
