...
from .core import (DatasetSummary, compute_quality_flags, correlation_matrix,
                   flatten_summary_for_print, missing_table, summarize_dataset,
                   top_categories)
from .viz import (plot_correlation_heatmap, plot_histograms_per_column,
                  plot_missing_matrix, save_top_categories_tables)
...
def report(
    ...
    if json_summary:
        summary_json = {
            ...
            "problematic_columns": {
                "missing": (
                    problem_missing_cols.index.tolist()
                    if not missing_df.empty
                    else []
                ),
                "constant": (
                    quality_flags.get("constant_columns", [])
                ),
            },
        }
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write("## Пропуски\n\n")
        f.write(f"Порог проблемных колонок по пропускам: **{min_missing_share:.0%}**\n\n")

        if missing_df.empty:
            f.write("Пропусков нет или датасет пуст.\n\n")
        else:
            if problem_missing_cols.empty:
                f.write("Колонок с большим числом пропусков не найдено.\n\n")
            else:
                f.write("Колонки с проблемными пропусками:\n\n")
                for col, row in problem_missing_cols.iterrows():
                    f.write(f"- `{col}` — {row['missing_share']:.1%}\n")
                f.write("\n")
